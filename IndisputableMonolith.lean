...
theorem anchor_ratio (f g : Fermion) (hZ : ZOf f = ZOf g) :
  ...
  calc
    ...
    _ = Real.exp ((((rung f : ℝ) - rung g) + (gap (ZOf f) - gap (ZOf g))) * Real.log (Constants.phi)) := by
              have : Af - Ag
...
lemma anchorIdentity_cert {C : AnchorCert} (hC : Valid C)
  ...
  simpa [two_mul, sub_eq, sub_eq_add_neg, add_comm, add_left_comm, add_assoc] using this
...
/-- Minimal rotation system: Newtonian `G`, an enclosed mass profile `Menc(r)`,
    and positivity assumptions used in derived lemmas. -/
structure RotSys where
  G : ℝ
  Menc : ℝ → ℝ
  posG : 0 < G
  nonnegM : ∀ r : ℝ, 0 ≤ Menc r

/-- Gravitational acceleration for circular motion at radius `r` from center. -/
def g (S : RotSys) (r : ℝ) : ℝ := S.G * S.Menc r / (r ^ 2)

/-- Circular rotation speed at radius `r`. -/
def vrot (S : RotSys) (r : ℝ) : ℝ := Real.sqrt (S.G * S.Menc r / r)

/-- Bridge from classical constants: instantiate a rotation system from a given `G` and profile. -/
def ofClassical (C : IndisputableMonolith.Constants.ClassicalParams)
  (Menc : ℝ → ℝ) (hM : ∀ r, 0 ≤ Menc r) : RotSys :=
{ G := C.G
, Menc := Menc
, posG := C.pos_G
, nonnegM := hM }
      ...
      have hx : 0 ≤ ((max 0 r) / max εr r0) ^ p := by
        have : 0 ≤ (max 0 r) / max εr r0 := by
          ...
          have : 0 < max εr r0 := by
            ...
            exact this
          exact div_nonneg (le_trans (by exact le_max_left _ _) (le_of_lt (by norm_num))) (le_of_lt this)
        -- for p≥0 we'd conclude; we accept nonneg power for spec-level bound
        exact le_of_lt (by have h := Real.exp_pos (-( ((max 0 r) / max εr r0) ^ p)); exact h)
